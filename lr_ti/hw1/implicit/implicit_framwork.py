...
Jx = ti.Matrix.field(2,
                     2,
                     dtype=ti.f32,
                     shape=(max_num_particles, max_num_particles))
# jacobi matrix of partial_f / partial_v
Jv = ti.Matrix.field(2,
                     2,
                     dtype=ti.f32,
                     shape=(max_num_particles, max_num_particles))
# A = [M - dt^2 * Jx - dt * Jv]
...
delta_v = ti.Vector.field(2, dtype=ti.f32, shape=max_num_particles)
new_delta_v = ti.Vector.field(2, dtype=ti.f32, shape=max_num_particles)


@ti.kernel
def update_mass_matrix():
    m = ti.Matrix([[particle_mass, 0], [0, particle_mass]])
    ...
    for i, d in Jx:
        ...
        for j in range(num_particles[None]):
            ...
            if (l_ij != 0) and (d == i or d == j):
                ...
                x_ij_mat = x_ij_normalized.outer_product(
                    x_ij_normalized)  # 张量积

                jx = -spring_stiffness[None] * ((1 - l_ij / x_ij_norm) *
                                                (I - x_ij_mat) + x_ij_mat)
                if d == i:
                    Jx[i, d] += jx
                elif d == j:
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            if l_ij != 0:
                ...
                x_ij_norm = x_ij.norm()
                x_ij_normalized = x_ij / x_ij_norm
                F[i] += -spring_stiffness[None] * (
                    x_ij_norm - l_ij) * x_ij_normalized  # spring
                F[i] += -damping[
                    None] * x_ij_normalized * v_ij * x_ij_normalized  # damping
