...
num_particles = ti.field(ti.i32, shape=())  # 质点数目
spring_stiffness = ti.field(ti.f32, shape=())  # 弹簧硬度
paused = ti.field(ti.i32, shape=())  # 是否暂停
damping = ti.field(ti.f32, shape=())  # 阻尼

particle_mass = 1  # 质点质量
bottom_y = 0.05  # 最底部y

x = ti.Vector.field(2, dtype=ti.f32, shape=max_num_particles)  # 质点位置
v = ti.Vector.field(2, dtype=ti.f32, shape=max_num_particles)  # 质点速度

# rest_length[i, j] = 0 means i and j are not connected
rest_length = ti.field(ti.f32, shape=(max_num_particles,
                                      max_num_particles))  # 两个质点之间的弹簧原长
...
# 计算能量
origin_energy = ti.field(ti.f32, shape=())  # 原始的能量，只有重力势能
current_energy = ti.field(ti.f32, shape=())  # 现在的能量，重力势能+弹性势能+动能
lost_energy = ti.field(ti.f32, shape=())  # 损失的能量，撞击地面损失的动能，转化为了热能
damp_energy = ti.field(ti.f32, shape=())  # 损失的能量，阻尼损失的动能，转化为了热能
    ...
    for i in range(n):
        ...
        for j in range(i, n):  # 防止计算两次
            if rest_length[i, j] != 0:
                x_ij = x[i] - x[j]
                # 1/2 k x^2
                current_energy[None] += 0.5 * spring_stiffness[None] * (
                    x_ij.norm() - rest_length[i, j])**2
    ...
    for i in range(n):
        ...
        for j in range(n):
            if rest_length[i, j] != 0:
                x_ij = x[i] - x[j]
                v_ij = v[i] - v[j]
                total_force += -damping[None] * x_ij.normalized(
                ) * v_ij * x_ij.normalized()  # damping
...
def process_output():
        ...
        for j in range(i + 1, num_particles[None]):
            if rest_length[i, j] != 0:
                ...
                gui.line(begin=X[i],
                         ...
                         radius=2,
                         color=calculate_color(norm - rest_length[i, j]))
    ...
    gui.text(content=f'D: damping {damping[None]:.2f}',
             pos=(0, 0.85),
             color=0x0)
    gui.text(content=f'Number of particles {num_particles[None]:.0f}',
             pos=(0, 0.80),
             color=0x0)
    gui.text(content=f'Origin energy {origin_energy[None]:.0f}',
             pos=(0, 0.75),
             color=0x0)
    gui.text(content=f'Current energy {current_energy[None]:.0f}',
             pos=(0, 0.70),
             color=0x0)
    gui.text(content=f'Lost energy {lost_energy[None]:.0f}',
             pos=(0, 0.65),
             color=0x0)
    gui.text(content=f'Damp energy {damp_energy[None]:.0f}',
             pos=(0, 0.60),
             color=0x0)
    gui.text(
        content=
        f'Total energy {current_energy[None] + lost_energy[None] + damp_energy[None]:.0f}',
        pos=(0, 0.55),
        color=0x0)
    gui.text(
        content=
        f'Error energy {origin_energy[None] - current_energy[None] - lost_energy[None] - damp_energy[None]:.0f}',
        pos=(0, 0.50),
        color=0x0)
    gui.show()
